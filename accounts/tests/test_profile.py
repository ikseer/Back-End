...
class UserProfileTest( APITestCase):
    ...
    #     self.assertIn('image', response.data)  # Assuming your API returns the image data under the 'image' key
    #     self.assertEqual(response.data['image'].name, 'image.jpg')  # Ensure the correct image name is returned
...
class MockingProfileTest( APITestCase):
    ...
    def test_serializer_contains_expected_fields(self, mock_save):
        mock_save.return_value = "test_profile.jpg"
        data = SimpleUploadedFile(
            name="test_profile.jpg", content=self.image
        )
        url=reverse('profile-detail', args=[self.user.profile.id])  # Assuming 'profile-detail' is the name generated by the router
        self.profile_data['user'] = self.user.id
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {self.access_token}')
    ...
    def test_serializer_contains_expected_fields(self, mock_save, ):
        ...
        mock_save.return_value = "test_profile.jpg"  # Mocking the save method to return the expected value

        updated_data = {
            'profile_image': SimpleUploadedFile(name='test_profile.jpg', content=b'mock image data', content_type='image/jpeg')
        }
        ...
        self.assertTrue(os.path.exists(os.path.join(settings.MEDIA_ROOT, "default/user.jpg")))
