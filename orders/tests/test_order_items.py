...
class OrderItemTest(TestCase): 
    def setUp(self) -> None:
        ...
        self.order=self.client.post('/orders/orders/', {'customer': user.id, 'pharmacy': self.pharmacy.id}, format='json').data
    ...
    def create_order_item(self):
        response = self.client.post('/orders/orderItem/', self.order_item_data, format='json')
    ...
    def test_update_order_item(self):
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {self.access_token}')
        order_item=self.client.post('/orders/orderItem/', self.order_item_data, format='json').data
        order_item_data={
            'id': order_item['id'],
            'product': self.product1.id,
            'quantity': 5,
        ...
        response = self.client.put('/orders/orderItem/'+str(order_item['id'])+'/', order_item_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
